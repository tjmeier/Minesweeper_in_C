...
unsigned int getPlayAgain(void) {
	...
	scanf_s("%u", &play_again);

	while (play_again != 'y' && play_again != 'n') {
		printf("\nTry again. Must be exactly the character 1 for yes and 0 for no.");
		...
		for (i = 0; i < height; i++) {
			...
			for (j = 0; j < length; j++) {
				...
				if ((p_current_Square->is_revealed) && (p_current_Square->qty_nearby_bombs == 0)/* && !(p_current_Square->function_revealNeighbors_was_ran)*/) { //checks if the current Square is revealed and has no nearby bombs i.e. is a 0 Square
					printf("\n\nexpandZeroSquares 1: Run Count %u: We see at (%u, %u) there is a revealed square with a zero value for nearby bombs that has never had it's neighbors expanded.\n",run_counter, i, j);
